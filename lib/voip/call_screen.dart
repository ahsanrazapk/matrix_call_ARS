...
class _CallScreenState extends State<CallScreen> with VivokaRouteCommands {
        ...
        children: [
          Expanded(
            child: Stack(
              ...
              children: [
                Positioned.fill(
                  child: Container(color: Colors.black,child:remoteScreenReady
                      ? RTCVideoView(
                        ...
                        child: Row(
                          ...
                          children: [
                            if (isIncoming && call.state == CallState.kRinging) ...[
                              ...
                              CallActionButton(onTap: () => voip.toggleMic(), icon: Icon(voip.micMuted ? Icons.mic_off : Icons.mic)),
                              if (isVideo)
                                CallActionButton(
                                  onTap: () => voip.toggleCam(),
                                  icon: Icon(voip.camMuted ? Icons.videocam_off : Icons.videocam),
                                ),
                              CallActionButton(
                                onTap: () async {
                                  await voip.hangup();
  ...
  bool onVivokaCommand(String cmd) {
    ...
    if (cmd == 'mute') { voip.muteMic(true); return true; }
    if (cmd == 'unmute') { voip.muteMic(false); return true; }
    ...
    if (cmd == 'zoom level one') { voip.setLocalCameraZoom(1.0); return true; }
    if (cmd == 'zoom level two') { voip.setLocalCameraZoom(2.0); return true; }
    if (cmd == 'zoom level three') { voip.setLocalCameraZoom(3.0); return true; }
    if (cmd == 'zoom level four') { voip.setLocalCameraZoom(4.0); return true; }
    if (cmd == 'zoom level five') { voip.setLocalCameraZoom(5.0); return true; }
